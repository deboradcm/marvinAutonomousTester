...
class QLearning:
    def __init__(self, states, actions, alpha=0.5, gamma=0.9, epsilon=1.0, min_epsilon=0.01, decay_rate=0.995, robot_id=1):
        ...
        logger.info(f'Algoritmo Q-Learning inicializado para o robô {robot_id}.')
...
class Robot:
    def __init__(self, robot_id, states, actions, button_position):
        self.robot_id = robot_id
        self.environment = Environment(states, button_position)
        self.q_learning = QLearning(states, actions, robot_id=robot_id)
        logger.info(f'Robô {robot_id} inicializado.')
...
if __name__ == "__main__":
    ...
    # Inicializa dois robôs
    robot1 = Robot(1, states, len(actions), Button.generate_position())
    robot2 = Robot(2, states, len(actions), robot1.environment.button_position)

    for episode in range(num_episodes):
        # Executa um episódio para cada robô
        total_reward1 = robot1.run_episode(max_steps_per_episode)
        total_reward2 = robot2.run_episode(max_steps_per_episode)

        # Registra as coordenadas dos robôs
        add_coordinates(1, episode, robot1.environment.current_state[0], robot1.environment.current_state[1])
        add_coordinates(2, episode, robot2.environment.current_state[0], robot2.environment.current_state[1])

        logger.info(f'Episódio {episode}: Recompensa total do robô 1: {total_reward1}')
        logger.info(f'Episódio {episode}: Recompensa total do robô 2: {total_reward2}')

        if robot1.environment.button_position_shared and robot2.environment.button_position_shared:
            ...
            robot1.environment.reset(new_button_position)
            robot2.environment.reset(new_button_position)
            robot2.environment.button_position_shared = robot1.environment.button_position_shared

    # Salva as Q-tables dos robôs
    np.save(os.path.join('notebooks', 'q_table_robot1.npy'), robot1.q_learning.q_table)
    np.save(os.path.join('notebooks', 'q_table_robot2.npy'), robot2.q_learning.q_table)

    logger.info(f'Posição final do botão compartilhado: {robot1.environment.button_position_shared}')

    log_button_position_shared(robot1.environment.button_position_shared)
    ...
    with open(os.path.join('data', 'button_press_records.csv'), 'w') as f:
        ...
        writer.writerow([1, 1 if robot1.environment.button_position_shared else 0])
        writer.writerow([2, 1 if robot2.environment.button_position_shared else 0])

    # Imprime a acurácia
    log_accuracy_table(robot1, robot2)
